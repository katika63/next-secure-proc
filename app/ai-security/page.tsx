...
export default function DynamicSentinelAIPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      sender: 'user',
      timestamp: '10:42 AM',
      content: 'Analyze CVE-2024-3094 and explain the potential impact and mitigation steps.'
    },
    {
      id: 'msg-2',
      sender: 'ai',
      timestamp: '10:42 AM',
      content: "Here's my analysis of CVE-2024-3094:",
      cveData: {
        cveId: 'CVE-2024-3094',
        overview: 'CVE-2024-3094 is a critical vulnerability in XZ Utils allowing attackers to execute arbitrary code via SSH authentication hooks.',
        severity: 'CRITICAL',
        cvssScore: '10.0 (CRITICAL)',
        potentialImpact: [
          'Unauthenticated remote code execution',
          'Full root system compromise',
          'SSH credential bypass',
          'Privilege escalation'
        ],
        affectedSystems: { product: 'XZ Utils / liblzma', versions: '5.6.0 - 5.6.1' },
        mitigationSteps: [
          'Downgrade XZ Utils to version 5.4.x immediately',
          'Audit SSH access logs for anomalous authentication attempts',
          'Isolate affected Linux instances from public internet'
        ],
        sources: ['NIST NVD', 'MITRE ATT&CK', 'CISA Alert']
      }
    }
  ])
  ...
  const handleTerminalCommand = async (e: React.FormEvent) => {
    ...
    if (networkCmds.includes(baseCmd)) {
      ...
      try {
        ...
        setTerminalLogs(prev => [...prev, data.output || 'Command executed.'])
      } catch (err: any) {
        setTerminalLogs(prev => [...prev, `[-] Proxy execution failed: ${err.message}`])
      } finally {
        setTerminalLoading(false)
      }
      return
    ...
    try {
      ...
      const data = await res.json()
      setTerminalLogs(prev => [...prev, data.response || 'Command executed.'])
    } catch {
      setTerminalLogs(prev => [...prev, '[-] Sentinel API connection lost.'])
    } finally {
      setTerminalLoading(false)
    }
  }
  ...
  const handleRunPlaybook = async (pb: Playbook) => {
    setPlaybooks(prev => prev.map(p => p.id === pb.id ? { ...p, status: 'Executing' } : p))
    try {
      const res = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: pb.prompt, mode: 'chat' })
      })
      const data = await res.json()
      setPlaybooks(prev => prev.map(p => p.id === pb.id ? { ...p, status: 'Completed', result: data.response } : p))
    } catch {
      setPlaybooks(prev => prev.map(p => p.id === pb.id ? { ...p, status: 'Completed', result: 'Failed to run playbook.' } : p))
    }
  }
  ...
  const handleTabAnalysis = async () => {
    ...
    try {
      const res = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: `Perform deep ${activeTab} analysis on: ${tabQuery}`, mode: 'chat' })
      })
      const data = await res.json()
      setTabResult(data.response)
    } catch {
      setTabResult('Analysis failed.')
    } finally {
      setTabLoading(false)
    }
  }

  // Load Live Vulnerabilities (CISA Known Exploited Vulnerabilities Feed)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-100 font-sans selection:bg-blue-600 selection:text-white relative overflow-hidden cyber-grid-bg">
      ...
      {/* ─── TOP HEADER BAR ────────────────────────────────────────────────────────── */}
      <header className="h-16 border-b border-[#0f1d38] bg-[#040a17]/95 backdrop-blur-xl sticky top-0 z-50 px-6 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <Link href="/" className="flex items-center space-x-2 group">
            <i className="fas fa-shield-alt text-blue-500 text-2xl group-hover:scale-105 transition-transform" />
          ...
          <div>
            <div className="text-xs font-bold tracking-wider text-white font-mono uppercase">SENTINEL AI</div>
            <div className="text-[10px] text-slate-400 font-mono tracking-tight">AI SECURITY INTELLIGENCE</div>
          </div>
        </div>

        <nav className="hidden md:flex items-center space-x-6 text-xs font-medium text-slate-300 font-mono">
          {['Chat', 'Threat Intelligence', 'Vulnerabilities', 'Incidents', 'Research Lab', 'Playbooks'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`hover:text-white transition-colors ${activeTab === tab ? 'text-blue-400 font-bold' : ''}`}>{tab === 'Research Lab' ? 'Research' : tab}</button>
          ))}
        </nav>

        <div className="flex items-center space-x-4">
          <div className="hidden sm:flex items-center space-x-2 text-[11px] font-mono text-slate-400 bg-[#09152b] border border-[#14284d] px-3 py-1 rounded-lg">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span>IP: {visitorIp}</span>
          </div>
          <Link href="/client-portal" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#0a152d] border border-[#1e325c] hover:border-blue-500/50 text-slate-200 text-xs font-medium transition-all">
          ...
          <div className="w-8 h-8 rounded-full bg-gradient-to-r from-blue-600 to-cyan-500 flex items-center justify-center text-xs font-bold text-white shadow-md border border-blue-400/40">AD</div>
        </div>
      </header>

      {/* ─── MAIN DASHBOARD CONTENT GRID ───────────────────────────────────────────── */}
      <div className="max-w-[1700px] mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <main className="lg:col-span-6 space-y-4 flex flex-col justify-start">
          ...
          <div className="bg-[#050c1e]/90 border border-[#0f1d38] rounded-xl p-6 shadow-xl relative overflow-hidden flex justify-between items-center">
            <div className="space-y-2 max-w-lg z-10">
              <div className="text-xs font-bold text-blue-400 tracking-wider font-mono uppercase">BTM SENTINEL AI</div>
              <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">{activeTab} Workspace</h1>
          ...
          {activeTab === 'Chat' && (
            <div className="flex flex-col" style={{ height: 'calc(100vh - 220px)' }}>
              ...
              <div ref={chatContainerRef} className="bg-[#050c1e]/90 border border-[#0f1d38] rounded-xl p-5 shadow-xl flex-1 min-h-0 overflow-y-auto space-y-6 mb-3 scrollbar-thin scrollbar-thumb-[#1e3b75] scrollbar-track-transparent">
                {messages.map(msg => (
                  <div key={msg.id} className="space-y-4">
                    {msg.sender === 'user' && (
                      <div className="flex justify-end items-start space-x-3">
                        <div className="max-w-md bg-[#0a1835] border border-[#1a2e57] rounded-2xl rounded-tr-none p-4 text-xs text-slate-100 shadow-md">
                          <p className="leading-relaxed">{msg.content}</p>
                        </div>
                        <div className="w-8 h-8 rounded-full bg-[#0a1835] border border-[#1a2e57] flex items-center justify-center text-xs">👤</div>
                      </div>
                    )}
                    {msg.sender === 'ai' && (
                      <div className="flex items-start space-x-3">
                        <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-xs font-bold text-white shadow-lg shrink-0">AI</div>
                        <div className="flex-1 bg-[#040916] border border-[#0f1d38] rounded-2xl rounded-tl-none p-5 text-xs text-slate-200 space-y-4 shadow-xl">
                          <div className="leading-relaxed whitespace-pre-line text-slate-300" dangerouslySetInnerHTML={{ __html: msg.content }} />
                          {msg.cveData && (
                            <div className="space-y-4 pt-2 border-t border-[#0e1b34]">
                              <div><div className="font-bold text-white mb-1">Overview</div><p>{msg.cveData.overview}</p></div>
                              <div className="flex gap-3 text-slate-300 font-mono"><span className="text-rose-400 font-bold px-2 py-0.5 bg-rose-950 rounded border border-rose-500">{msg.cveData.severity}</span> CVSS: {msg.cveData.cvssScore}</div>
                            </div>
                          )}
                        </div>
                      </div>
                ...
                {loading && <div className="text-slate-400 font-mono text-xs flex items-center gap-2"><div className="w-4 h-4 rounded-full border-2 border-blue-500 border-t-transparent animate-spin"></div>Sentinel AI reasoning...</div>}
              </div>

              {/* Fixed chat input at bottom */}
              <div className="bg-[#050c1e]/90 border border-[#0f1d38] rounded-xl p-3 shadow-xl relative shrink-0">
                <input
                  type="text"
                  value={inputQuery}
                  ...
                  className="w-full pl-4 pr-12 py-3 bg-[#030816] border border-[#122244] focus:border-blue-500 rounded-xl text-sm text-white placeholder-slate-500 outline-none"
                />
                <button onClick={() => handleSendMessage()} disabled={loading} className="absolute right-5 top-5 p-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg">
          ...
          {/* TERMINAL TAB */}
          {activeTab === 'Terminal' && (
            <div className="bg-[#020612] border border-[#0f1d38] rounded-xl p-5 shadow-2xl flex flex-col font-mono" style={{ height: 'calc(100vh - 220px)' }}>
              {/* Terminal header bar */}
              <div className="flex items-center justify-between pb-3 mb-2 border-b border-[#0f1d38] shrink-0">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-[10px] text-slate-500 ml-2 font-bold">sentinel@cyber-node-01 — bash</span>
                </div>
                <span className="text-[10px] text-cyan-500 font-mono">TLS 1.3 ENCRYPTED</span>
              </div>
              {/* Scrollable logs area */}
              <div ref={terminalLogsContainerRef} className="flex-1 min-h-0 overflow-y-auto space-y-1.5 text-xs text-emerald-400 pr-1 scrollbar-thin scrollbar-thumb-emerald-900 scrollbar-track-transparent">
              ...
              {/* Fixed input bar */}
              <form onSubmit={handleTerminalCommand} className="mt-3 pt-3 border-t border-[#0f1d38] flex items-center space-x-2 shrink-0">
                <span className="text-cyan-400 text-xs font-bold shrink-0">sentinel@security-node:~$</span>
                <input
                  type="text"
                  value={terminalInput}
                  onChange={(e) => setTerminalInput(e.target.value)}
                  placeholder="ping 8.8.8.8 | whois domain.com | nmap 1.1.1.1 | help..."
                  className="flex-1 bg-transparent border-none text-xs text-white outline-none font-mono placeholder-slate-600"
          ...
          {activeTab === 'Vulnerabilities' && (
            <div className="bg-[#050c1e]/90 border border-[#0f1d38] rounded-xl p-6 shadow-xl flex flex-col" style={{ height: 'calc(100vh - 220px)' }}>
              <div className="flex justify-between items-center border-b border-[#0f1d38] pb-3 mb-4 shrink-0">
                <div>
                  <h2 className="text-base font-bold text-white flex items-center gap-2">
                    <span>Live CISA Exploited Vulnerabilities Feed</span>
                    <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                  </h2>
                  <p className="text-[11px] text-slate-400 font-mono">Real-time threat feed verified by CISA & DHS</p>
                </div>
                <button onClick={() => { setLiveCves([]); setCveLoading(true) }} className="text-xs text-blue-400 hover:text-white font-mono">↺ Refresh Feed</button>
              </div>
              ...
              <div className="flex-1 min-h-0 overflow-y-auto space-y-3 pr-1 scrollbar-thin scrollbar-thumb-[#1e3b75] scrollbar-track-transparent">
                {cveLoading && (
                  <div className="text-slate-400 text-xs animate-pulse font-mono flex items-center gap-2">
                    ...
                    Connecting to CISA Real-Time Threat Feed...
                  </div>
                )}
                ...
                {liveCves.map((cve: any, idx: number) => {
                  ...
                  return (
                    <div key={displayId + idx} className="p-4 rounded-xl bg-[#030816] border border-[#102246] hover:border-blue-500/40 transition-colors">
                      <div className="flex justify-between items-center mb-2">
                        <div className="flex items-center space-x-2">
                          <span className="px-2 py-0.5 rounded bg-rose-950/80 border border-rose-500/40 text-rose-400 font-mono text-xs font-bold">{displayId}</span>
                          <span className="px-2 py-0.5 rounded bg-blue-950 text-blue-300 font-mono text-[10px] uppercase">{displayVendor}</span>
                        </div>
                      ...
                      {activeCveAnalysis?.id === displayId && activeCveAnalysis && (
                        <div className="mt-4 p-4 rounded-lg bg-[#050b1a] border border-[#102246] text-xs text-slate-300 space-y-2">
                          {activeCveAnalysis.content === 'Analyzing...' ? (
                            <div className="animate-pulse flex items-center gap-2 font-mono text-cyan-400">
                              <div className="w-3 h-3 rounded-full border-2 border-blue-500 border-t-transparent animate-spin" />
                              Sentinel AI analyzing risk & TTPs...
                            </div>
                          ) : (
                            <>
                              <div dangerouslySetInnerHTML={{ __html: activeCveAnalysis.content }} className="space-y-2 leading-relaxed" />
                              {activeCveAnalysis.data && (
                                <div className="mt-3 pt-3 border-t border-[#122244] font-mono text-[10px] text-amber-400 flex items-center justify-between">
                                  <span>CVSS RISK SCORE: {activeCveAnalysis.data.cvssScore || '9.8 (CRITICAL)'}</span>
          ...
          {/* PLAYBOOKS TAB */}
          {activeTab === 'Playbooks' && (
            <div className="bg-[#050c1e]/90 border border-[#0f1d38] rounded-xl p-5 shadow-xl flex flex-col" style={{ height: 'calc(100vh - 220px)' }}>
              <h2 className="text-base font-bold text-white mb-4 border-b border-[#0f1d38] pb-3 shrink-0">Automated Security Playbooks</h2>
              ...
              <div className="space-y-4">
                {playbooks.map(pb => (
                  <div key={pb.id} className="p-4 rounded-xl bg-[#030816] border border-[#102246] space-y-3">
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="px-2 py-0.5 rounded bg-blue-950 text-blue-300 font-mono text-[10px] uppercase">{pb.category}</span>
                        <h3 className="text-sm font-bold text-white mt-1">{pb.title}</h3>
                      </div>
                    ...
                    <p className="text-xs text-slate-300">{pb.description}</p>
                    ...
                    {pb.result && (
                      <div className="mt-4 p-4 rounded-lg bg-[#050b1a] border border-[#102246] text-xs text-slate-300">
                        <div dangerouslySetInnerHTML={{ __html: pb.result }} className="space-y-2 leading-relaxed" />
                      </div>
                    )}
                  </div>
                ))}
          ...
          {['Threat Intelligence', 'Incidents', 'Research Lab'].includes(activeTab) && (
            <div className="bg-[#050c1e]/90 border border-[#0f1d38] rounded-xl p-6 shadow-xl flex flex-col" style={{ height: 'calc(100vh - 220px)' }}>
              <h2 className="text-base font-bold text-white mb-4 border-b border-[#0f1d38] pb-3 shrink-0">{activeTab} Workspace</h2>
              ...
              <div className="flex space-x-2 mb-6">
                <input
                  ...
                  className="flex-1 px-4 py-2 bg-[#030816] border border-[#122244] rounded-lg text-xs text-white outline-none"
                  onKeyDown={e => e.key === 'Enter' && handleTabAnalysis()}
                />
                <button onClick={handleTabAnalysis} disabled={tabLoading} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-xs font-bold font-mono">
                  {tabLoading ? 'Analyzing...' : 'Analyze'}
                </button>
              </div>

              <div className="flex-1 overflow-y-auto bg-[#030816] rounded-xl border border-[#102246] p-5">
                {tabLoading ? (
                  <div className="animate-pulse flex items-center gap-2 text-slate-400 text-xs font-mono"><div className="w-4 h-4 rounded-full border-2 border-blue-500 border-t-transparent animate-spin"></div>Sentinel AI generating report...</div>
                ) : tabResult ? (
                  <div dangerouslySetInnerHTML={{ __html: tabResult }} className="text-xs text-slate-300 leading-relaxed space-y-3" />
                ) : (
                  <div className="text-xs text-slate-500 font-mono text-center mt-10">Awaiting query to generate {activeTab} analysis report...</div>
                )}
              </div>
            </div>
          )}
        ...
        <aside className="lg:col-span-3 space-y-5">
          <div className="bg-[#050c1e]/90 border border-[#0f1d38] rounded-xl p-5 shadow-xl space-y-4">
            ...
            <div className="flex flex-col items-center justify-center py-2">
              <div className="relative w-24 h-24 flex items-center justify-center">
                ...
                <div className="absolute flex flex-col items-center">
                  <span className="text-2xl font-extrabold text-white font-mono">{securityScore}</span>
